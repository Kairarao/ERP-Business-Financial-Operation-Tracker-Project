...
class productClass:
    def __init__(self,root):
        ...
        # Frames
        palette.apply_to_window(product_Frame)
        palette.apply_to_window(SearchFrame)
        palette.apply_to_window(product_frame)

        # Labels
        palette.apply_to_label(title)
        palette.apply_to_label(lbl_category)
        palette.apply_to_label(lbl_supplier)
        palette.apply_to_label(lbl_product_name)
        palette.apply_to_label(lbl_price)
        palette.apply_to_label(lbl_qty)
        palette.apply_to_label(lbl_status)

        # Comboboxes
        """palette.apply_to_combobox(cmb_cat)
        palette.apply_to_combobox(cmb_sup)
        palette.apply_to_combobox(cmb_status)
        palette.apply_to_combobox(cmb_search)"""

        # Entries
        palette.apply_to_entry(txt_name)
        palette.apply_to_entry(txt_price)
        palette.apply_to_entry(txt_qty)
        palette.apply_to_entry(txt_search)

        # Buttons
        palette.apply_to_button(btn_add, 'active')
        palette.apply_to_button(btn_update, 'active')
        palette.apply_to_button(btn_delete, 'active')
        palette.apply_to_button(btn_clear, 'active')
        palette.apply_to_button(btn_search, 'active')

        # Scrollbars (styled as sliders)
        palette.apply_to_slider(scrolly)
        palette.apply_to_slider(scrollx)

        # Treeview
        palette.apply_to_treeview(self.ProductTable)
    ...
    def update(self):
        ...
        try:
            if self.var_pid.get()=="":
                ...
                cur.execute("Select * from product where pro_id=%s",(self.var_pid.get(),))
                row=cur.fetchone()
                if row==None:
                    messagebox.showerror("Error","Invalid Product",parent=self.root)
                else:
                    cur.execute("update product set Category=%s,name=%s,price=%s,qty=%s,status=%s,Supplier=%s,pro_id=%s where pro_id=%s",
                                (
                        self.var_cat.get(),
                        self.var_name.get(),
                        self.var_price.get(),
        ...
        try:
            if self.var_pid.get()=="":
                ...
                if row==None:
                    ...
                    if op==True:
                        cur.execute("delete from product where pro_id=%s",(self.var_pid.get(),))
                        self.con.commit()
